...
const labels: Record<string, string> = {
  answer: "Respuesta del alumno/a", challenge_prompt: "Enunciado del reto", initial_analysis: "AnÃ¡lisis inicial", objective: "Objetivo", secondary_objective: "Segundo objetivo", priority_capacity: "Capacidad prioritaria", duration_weeks: "Semanas", weekly_frequency: "Frecuencia semanal", session_duration_minutes: "Minutos por sesiÃ³n", progression_strategy: "ProgresiÃ³n", recovery_strategy: "RecuperaciÃ³n", success_indicator: "Indicador de logro", secondary_success_indicator: "Indicador del segundo objetivo", final_conclusions: "Conclusiones", future_work: "Trabajo futuro", plan_items: "Tareas del plan", activity: "Actividad", exercises: "Ejercicios", methodology: "MetodologÃ­a", rounds: "Vueltas", roundRecovery: "Pausa entre vueltas", dose: "Carga", recovery: "Descanso", sessionDate: "Fecha de la sesiÃ³n", day: "DÃ­a", goal: "Objetivo", capacity: "Capacidad", description: "DescripciÃ³n", title: "TÃ­tulo", activities: "Actividades", session_date: "Fecha de la sesiÃ³n", feeling: "Sensaciones", learning: "Aprendizajes", reflection: "ReflexiÃ³n", perceived_difficulty: "Dificultad percibida", perceived_effort: "Esfuerzo percibido", training_plan: "Trabajo de la sesiÃ³n", content_worked: "Contenido trabajado", exercises_performed: "Ejercicios realizados", participation: "ParticipaciÃ³n", performance_perception: "PercepciÃ³n del rendimiento", needs_improvement: "Aspectos a mejorar", session_number: "SesiÃ³n", week_number: "Semana", duration_minutes: "DuraciÃ³n (min)", rpe: "Esfuerzo percibido (RPE)", completion_percent: "RealizaciÃ³n declarada (%)", modifications: "Adaptaciones", period: "Toma", value: "Marca", quiz_score: "Mejor resultado (%)", completed: "Lectura completada", focus_habit: "HÃ¡bito", initial_reflection: "ReflexiÃ³n inicial", action_plan: "Acciones", barrier_strategy: "Dificultades y respuesta", final_reflection: "ReflexiÃ³n final", sleep_routine: "Rutina de sueÃ±o", daily_movement: "Movimiento diario", sedentary_breaks: "Pausas activas", hydration: "HidrataciÃ³n", balanced_eating: "AlimentaciÃ³n", goal_completion: "Cumplimiento del objetivo", next_adjustment: "PrÃ³ximo ajuste", event_name: "Evento", purpose: "PropÃ³sito", target_audience: "Destinatarios", activity_type: "Actividad", event_format: "Formato", venue: "Lugar", event_date: "Fecha", participants_estimate: "Participantes previstos", team_name: "Equipo", personal_role: "Rol personal", team_roles: "Roles", rules: "Normas", inclusion_measures: "InclusiÃ³n", safety_measures: "Seguridad", materials: "Materiales", budget_items: "Presupuesto", timeline: "Calendario", communication_plan: "ComunicaciÃ³n", sustainability_measures: "Sostenibilidad", evaluation_plan: "EvaluaciÃ³n", instrument_code: "Cuestionario", assessment_phase: "Momento", total_score: "PuntuaciÃ³n", attempt_number: "Intento", status: "Estado",
};
...
function DetailValue({ value }: { value: unknown }) {
  ...
  return <>{statuses[word] ?? ({ september: "Septiembre", december: "Diciembre", initial: "Inicial", final: "Final", circuit: "Circuit training", total: "Total training", custom: "Trabajo especÃ­fico" } as Record<string, string>)[word] ?? word}</>;
}
...
export function TeacherActivityDashboard({ data: initialData, year, initialSituation = "" }: { data: TeacherActivityData; year: 1 | 2; initialSituation?: string }) {
  ...
  function refresh() {
    ...
    startRefresh(async () => {
      try {
        ...
        setRefreshMessage("No se han podido actualizar los datos. Comprueba tu conexiÃ³n y vuelve a intentarlo. Si tu sesiÃ³n ha caducado, vuelve a entrar.");
      }
    });
  }
  ...
  return <div className="space-y-6">
    <section className="card p-5 sm:p-7">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-[#1e6b4f]">Seguimiento docente Â· {year}Âº Bachillerato</p><h1 className="mt-2 text-2xl font-extrabold">Actividad y resultados del alumnado</h1><p className="mt-2 text-sm text-slate-500">{data.courseName} Â· Datos guardados en la plataforma. Solo el profesorado puede consultar esta vista.</p></div><button onClick={refresh} disabled={refreshing || changingCourse} aria-busy={refreshing} className="rounded-xl border px-4 py-2 text-sm font-bold disabled:opacity-50">{refreshing ? "Actualizandoâ€¦" : "Actualizar datos"}</button></div>
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <label className="text-sm font-bold">Curso de los resultados<select aria-label="Curso de los resultados" value={year} disabled={changingCourse || refreshing} onChange={(event) => { const value = event.target.value; startCourseChange(() => router.push(`/profesor?curso=${value}`)); }} className="mt-2 w-full rounded-xl border border-emerald-600 bg-emerald-50 p-3"><option value="1">1Âº Bachillerato</option><option value="2">2Âº Bachillerato</option></select></label>
        ...
        <label className="text-sm font-bold">SituaciÃ³n<select value={sa} onChange={(event) => { const value = event.target.value; if (initialSituation) { startCourseChange(() => router.push(value ? `/profesor/${year === 2 ? "2bach/" : ""}${value.toLowerCase()}` : `/profesor?curso=${year}`)); } else { setSa(value); } }} disabled={changingCourse || refreshing} className="mt-2 w-full rounded-xl border p-3"><option value="">Todas las situaciones</option>{data.situations.map((entry) => <option key={entry.id} value={entry.code}>{entry.code} Â· {entry.title}</option>)}</select></label>
      ...
      <p className="mt-3 text-xs text-slate-500">Ãšltima consulta: {new Intl.DateTimeFormat("es-ES", { timeZone: "Europe/Madrid", dateStyle: "short", timeStyle: "medium" }).format(new Date(data.loadedAt))}</p>
      ...
      {changingCourse && <p role="status" className="mt-3 font-semibold text-[#1e6b4f]">Cargando los resultados del curso seleccionadoâ€¦</p>}
      <p className="mt-4 text-xs text-slate-500">{students.length} alumnos en el filtro Â· {activities.length} registros. No se calcula una nota global mezclando actividades de naturaleza distinta.</p>
    </section>
    {!students.length ? <p className="card p-6">No hay alumnado activo en este grupo.</p> : <>
      ...
      })}{["Esfuerzo percibido", "RealizaciÃ³n declarada (%)"].map((label) => {
      ...
      <section className="card space-y-4 p-5 sm:p-7"><h2 className="text-xl font-extrabold">Resultados de autoevaluaciÃ³n teÃ³rica</h2><p className="text-sm text-slate-500">Se utiliza la mejor puntuaciÃ³n guardada de cada tema. La media de grupo da el mismo peso a cada alumno que ha realizado algÃºn test; los pendientes no se cuentan como cero.</p>{view === "individual" && <Bar label={`Resultado medio Â· ${individualScores.length} tests`} value={mean(individualScores)} suffix="%"/>}<Bar label={`Media del grupo Â· ${groupScores.length} alumnos con tests`} value={mean(groupScores)} suffix="%" color="bg-sky-500"/><p className="text-sm">{view === "individual" ? `${individualScores.filter((score) => score >= 67).length} de ${individualScores.length} tests realizados superados (mÃ­nimo 67%).` : `${activities.filter((row) => row.category === "quizzes" && (row.score ?? -1) >= 67).length} tests superados en el grupo (mÃ­nimo 67%).`}</p></section>
      {(!sa || sa === "SA1") && <section className="card space-y-5 p-5 sm:p-7"><h2 className="text-xl font-extrabold">EvoluciÃ³n fÃ­sica por capacidades</h2><p className="text-sm text-slate-500">Septiembre y diciembre en las unidades de cada prueba. Cada media indica cuÃ¡ntos registros incluye; la comparaciÃ³n emparejada utiliza Ãºnicamente alumnos con ambas tomas.</p>{["Fuerza", "Resistencia", "Velocidad", "Flexibilidad / movilidad", "Otras pruebas"].map((capacity) => {
        ...
        return tests.length ? <div key={capacity}><h3 className="mb-3 font-extrabold text-[#1e6b4f]">{capacity}</h3><div className="grid gap-4 md:grid-cols-2">{tests.map((test) => {
          ...
          return <article key={test.id} className="space-y-3 rounded-xl border p-4"><h4 className="font-bold">{test.name} Â· {test.unit}</h4>{view === "individual" && <><Bar label="Septiembre Â· alumno/a" value={values.first} max={max} suffix={` ${test.unit}`}/><Bar label="Diciembre Â· alumno/a" value={values.last} max={max} suffix={` ${test.unit}`} color="bg-violet-500"/><p className="text-xs">Mejora entre tomas: {fmt(values.improvement)}{values.improvement !== null ? "%" : ""}</p></>}<Bar label={`Media septiembre Â· n=${values.initialN}`} value={values.initialMean} max={max} suffix={` ${test.unit}`} color="bg-sky-500"/><Bar label={`Media diciembre Â· n=${values.finalN}`} value={values.finalMean} max={max} suffix={` ${test.unit}`} color="bg-orange-400"/><p className="text-xs text-slate-500">Mismos {values.pairedN} alumnos en ambas tomas: {fmt(values.pairedInitial)} â†’ {fmt(values.pairedFinal)} {test.unit}. {test.direction === "lower_better" ? "Menor marca indica mejor resultado." : "Mayor marca indica mejor resultado."}</p></article>;
        })}</div></div> : null;
      })}{!activities.some((row) => row.category === "physical") && <p>Sin marcas fÃ­sicas registradas en este filtro.</p>}</section>}
      <section className="card overflow-hidden p-5 sm:p-7"><h2 className="text-xl font-extrabold">Resumen de cada alumno/a</h2><p className="mt-2 text-sm text-slate-500">NÃºmero de registros por apartado. Pulsa un nombre para consultar el trabajo completo.</p><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-xs"><thead><tr><th className="p-3">Alumno/a</th>{categories.map(([key, label]) => <th key={key} className="p-3">{label}</th>)}</tr></thead><tbody>{students.map((student) => <tr key={student.id} className="border-t"><td className="p-3"><button className="font-bold text-[#1e6b4f] underline" onClick={() => { setStudentId(student.id); setView("individual"); }}>{student.name}</button></td>{categories.map(([key]) => <td key={key} className="p-3">{activities.filter((row) => row.studentId === student.id && row.category === key).length}</td>)}</tr>)}</tbody></table></div></section>
      <section className="card space-y-4 p-5 sm:p-7"><h2 className="text-xl font-extrabold">{view === "individual" ? `Trabajo de ${selected!.name}` : "Detalle de los registros del grupo"}</h2><p className="text-sm text-slate-500">Los planes reflejan lo previsto y su estado declarado; los diarios reflejan lo registrado tras la prÃ¡ctica. Abre cada apartado para leer objetivos, ejercicios, cargas, reflexiones y resultados.</p>{categories.map(([category, label]) => {
        const rows = details.filter((row) => row.category === category).sort((a, b) => b.date.localeCompare(a.date));
        return <details key={category} className="rounded-xl border p-4"><summary className="cursor-pointer font-bold">{label} Â· {rows.length} registros</summary><div className="mt-4 space-y-3">{!rows.length && <p className="text-sm text-slate-500">Sin registros guardados.</p>}{rows.map((row) => <details key={row.id} className="rounded-lg bg-slate-50 p-4"><summary className="cursor-pointer text-sm font-semibold">{view === "group" ? `${students.find((student) => student.id === row.studentId)?.name} Â· ` : ""}{row.sa} Â· {row.title} {row.status ? `Â· ${statuses[row.status] ?? row.status}` : ""}{row.score !== null ? ` Â· ${fmt(row.score)} ${row.scoreLabel ?? ""}` : ""}</summary><div className="mt-4 text-sm"><p className="mb-3 text-xs text-slate-500">Ãšltimo registro: {row.date ? new Date(row.date).toLocaleDateString("es-ES") : "Sin fecha"}</p><DetailValue value={row.details}/></div></details>)}</div></details>;
      })}</section>
      {(!sa || sa === "SA1") && <section className="space-y-4"><h2 className="text-xl font-extrabold">Informes psicolÃ³gicos Â· inicial y final</h2><TeacherPsychologicalReports key={`${year}-${group}-${selected?.id}-${data.loadedAt}`} initialYear={year} initialStudentId={selected?.id} initialGroup={selected?.group}/></section>}
    </>}
  </div>;
}
