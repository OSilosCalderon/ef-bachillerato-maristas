...
export function SecondYearTheoryReader({ initialSlug, compactHeader = false }: Props) {
  ...
  const mastery = Math.round(((readCount + passedCount) / (secondYearTheoryTopics.length * 2)) * 100);
  ...
  return <div className="space-y-6">
    ...
    <div className="lg:hidden"><label htmlFor="second-year-topic" className="mb-2 block text-xs font-black uppercase tracking-[.14em] text-slate-500">Módulos de 2º</label><select id="second-year-topic" value={topic.slug} onChange={(event) => { setSelectedSlug(event.target.value); setMessage(""); }} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 shadow-sm">{secondYearTheoryTopics.map((item) => <option key={item.slug} value={item.slug}>{item.sa} · {item.title}</option>)}</select></div>

    <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="hidden lg:block lg:sticky lg:top-5 lg:self-start"><nav aria-label="Módulos teóricos de 2º" className="card p-3"><p className="px-2 pb-2 text-xs font-black uppercase tracking-[.14em] text-slate-400">Módulos</p><div className="space-y-1">{secondYearTheoryTopics.map((item) => { const active=item.slug===topic.slug; const read=Boolean(progress[item.slug]?.completed); const passed=(progress[item.slug]?.quizScore??0)>=PASS_SCORE; return <button key={item.slug} type="button" onClick={()=>{setSelectedSlug(item.slug);setMessage("");}} className={`w-full rounded-xl px-3 py-3 text-left transition ${active?"bg-[#e7f2ed] text-[#164c3a]":"text-slate-600 hover:bg-slate-50"}`}><div className="flex items-start gap-3"><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg text-xs font-black ${active?"bg-[#1e6b4f] text-white":"bg-slate-100 text-slate-600"}`}>{item.number}</span><div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">{item.sa}</p><p className="text-sm font-extrabold leading-5">{item.title}</p><p className="mt-1 text-[10px] font-bold text-slate-400">{read?"Leído":"Pendiente"} · {passed?"Test superado":"Test pendiente"}</p></div></div></button>})}</div></nav></aside>
      ...
      <article className="min-w-0 space-y-6">
        ...
        <section className="card overflow-hidden"><div className="aspect-[16/8] overflow-hidden bg-slate-50"><SecondYearTheoryVisual kind={topic.visual} alt={`Infografía del módulo ${topic.number}: ${topic.title}`}/></div><div className="p-5 sm:p-6"><p className="text-xs font-black uppercase tracking-wide text-[#1e6b4f]">Imagen para comprender</p><p className="mt-2 text-sm font-semibold leading-6 text-slate-600">{topic.visualCaption}</p></div></section>
