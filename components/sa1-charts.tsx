...
type EvolutionDatum = { label: string; september: number; december: number };

export function ComparisonBarChart({ data }: { data: EvolutionDatum[] }) {
  return (
    <div className="h-72 w-full" aria-label="Comparativa visual septiembre y diciembre">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 8, right: 8, bottom: 12, left: -16 }}>
          ...
          <Bar dataKey="december" name="Diciembre" fill="#1e6b4f" radius={[6, 6, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
...
export function EvolutionLineChart({ data }: { data: EvolutionDatum[] }) {
  const flattened = data.flatMap((item) => [
    ...
    { point: `${item.label} · Dic`, value: item.december },
  ]);
  return (
