  ...
  return <div className="space-y-4 md:col-span-2">
    ...
    {item.exercises.map((exercise, index) => <fieldset key={exercise.id} className="min-w-0 rounded-xl border border-slate-200 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="text-sm font-bold">Trabajo por serie · tiempo, repeticiones e intensidad<input value={exercise.dose} onChange={(event) => edit(exercise.id, { dose: event.target.value })} placeholder="30 s · esfuerzo 5/10, u 8 repeticiones" className={field}/></label>
