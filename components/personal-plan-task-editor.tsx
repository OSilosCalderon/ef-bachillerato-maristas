...
export function PersonalPlanTaskEditor({ item, onChange }: { item: PlanItem; onChange: (item: PlanItem) => void }) {
  ...
  return <div className="space-y-4 md:col-span-2">
    ...
    {item.exercises.map((exercise, index) => <fieldset key={exercise.id} className="min-w-0 rounded-xl border border-slate-200 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm font-bold sm:col-span-2">Ejercicio del catálogo<select value={exercise.exerciseId} onChange={(event) => select(exercise.id, event.target.value)} className={field}><option value="">Ejercicio propio</option>{capacities.map(([value, label]) => <optgroup key={value} label={label}>{secondYearExercises.filter((entry) => entry.capacity === value).map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</optgroup>)}</select></label>
        ...
        <label className="text-sm font-bold">Actividad<input value={exercise.activity} onChange={(event) => edit(exercise.id, { activity: event.target.value })} className={field}/></label>
