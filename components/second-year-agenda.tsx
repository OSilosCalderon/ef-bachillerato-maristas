...
type Session = { id: string; session_date: string; start_time: string; end_time: string; status: string; note: string | null };
...
export function SecondYearAgenda({ teacher = false }: { teacher?: boolean }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const { data, error } = await createClient().from("class_sessions").select("id,session_date,start_time,end_time,status,note")
          .eq("academic_year", SECOND_YEAR_AGENDA_YEAR).eq("class_group", SECOND_YEAR_AGENDA_GROUP).order("session_date").order("start_time");
        if (error) throw error;
        if (active) setSessions(data ?? []);
      } catch { if (active) setError("No se ha podido cargar la agenda. Recarga para volver a intentarlo."); }
  ...
  return <section className="card space-y-5 p-5 sm:p-8">
    <div><h2 className="text-2xl font-extrabold">Agenda de clases · 2º Bachillerato</h2><p className="mt-2 text-sm leading-6 text-slate-600">Lunes y miércoles 10:05–11:00 · Martes y jueves 13:20–14:15. Sesiones de 55 minutos; cuatro clases no significan cuatro entrenamientos intensos consecutivos.</p></div>
    <p className="rounded-xl bg-emerald-50 p-4 text-sm leading-6 text-emerald-900">Distribución orientativa: lunes, fuerza y técnica; martes, resistencia moderada y movilidad; miércoles, velocidad y saltos con recuperación; jueves, práctica deportiva y revisión del plan. Reserva tiempo para calentamiento, organización y vuelta a la calma. La planificación del profesor en cada fecha tiene prioridad.</p>
    ...
    <p className="text-xs leading-5 text-slate-500">Calendario 2026/2027: excluye festivos autonómicos, Navidad, Carnaval y Semana Santa. 2º termina las clases el 7 de mayo. El profesor puede cancelar clases por ajustes locales o del centro. <a className="underline" href="https://doe.juntaex.es/otrosFormatos/html.php?anio=2026&doe=1160o&xml=2026061567" target="_blank" rel="noreferrer">Calendario oficial de Extremadura</a>.</p>
  </section>;
}
  ...
  async function save() {
    ...
    try {
      const { data, error } = await createClient().from("class_sessions").update({ start_time: draft.start_time, end_time: draft.end_time, status: draft.status, note: draft.note?.trim() ?? "" })
        .eq("id", session.id).eq("class_group", SECOND_YEAR_AGENDA_GROUP).select("id,session_date,start_time,end_time,status,note").single();
