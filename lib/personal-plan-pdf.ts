...
export function buildPersonalPlanPdf(data: PlanPdfData) {
  ...
  objects.set(5, `<< /Type /XObject /Subtype /Image /Width 360 /Height 120 /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${logo.length} >>\nstream\n${logo}\nendstream`);
  pages.forEach((lines, index) => {
    const contentId = 6 + index * 2;
    const pageId = 7 + index * 2;
    let y = TOP;
    const commands = ["q 108 0 0 36 437 798 cm /Logo Do Q", "0.08 0.17 0.15 rg", "0.06 0.46 0.43 RG", "2 w", "48 790 m 547 790 l S"];
    ...
    objects.set(pageId, `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> /XObject << /Logo 5 0 R >> >> /Contents ${contentId} 0 R >>`);
  });
  const lastId = 5 + pages.length * 2;
  let pdf = "%PDF-1.4\n";
  const offsets = [0];
