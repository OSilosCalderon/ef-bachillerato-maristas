...
export async function loadTeacherActivity(year: 1 | 2): Promise<TeacherActivityData> {
  ...
  const students = activeStudents.map((row) => {
    ...
    return { id: str(row.id), name: str(profile?.display_name) || "Alumno/a", group: firstYearCalendarGroup(str(profile?.class_group)) ?? (str(profile?.class_group) || `${year}Âº Bachillerato`) };
  ...
  for (const row of table.training_session_logs) add(row, "journals", `SesiÃ³n de entrenamiento ${row.session_number}`, str(row.sa_code), num(row.completion_percent), "RealizaciÃ³n declarada (%)");
  ...
  for (const row of table.theory_topic_progress) {
    ...
    if (row.quiz_score !== null) add(row, "quizzes", topic.title, topic.sa, num(row.quiz_score), "Mejor puntuaciÃ³n (%)");
  }
