...
export default async function StudentDashboard() {
    ...
    return <>
      ...
      <div className="mx-auto max-w-7xl space-y-8 p-5 sm:p-8">
        <section className="card overflow-hidden">
          ...
          <div className="p-6"><Link href="/alumno/apuntes-entrenamiento" className="inline-flex items-center gap-2 rounded-xl bg-[#1e6b4f] px-4 py-2.5 text-sm font-bold text-white"><GraduationCap size={18}/>Abrir los 8 temas interactivos</Link></div>
        </section>
  ...
  return <>
    ...
    <div className="mx-auto max-w-7xl space-y-8 p-5 sm:p-8">
      ...
      <section className="card flex flex-col gap-5 p-6 md:flex-row md:items-center md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#1e6b4f]">Biblioteca común de Bachillerato</p><h2 className="mt-1 text-xl font-extrabold">8 temas interactivos de entrenamiento</h2><p className="mt-2 text-sm text-slate-500">Principios, carga, fuerza y contracciones, resistencia, velocidad, planificación y periodización.</p></div><Link href="/alumno/apuntes-entrenamiento" className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white"><GraduationCap size={18}/>Estudiar temas</Link></section>
