    ...
    <div className="mx-auto max-w-7xl space-y-8 p-5 sm:p-8">
      ...
      <section>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {weeklyItems.filter((item) => visible.includes(item.situation)).map((item)=><article key={item.id} className="card flex items-start gap-4 p-5"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#e7f2ed] text-[#1e6b4f]">{item.type.includes("Diario")?<NotebookPen size={18}/>:item.type.includes("Contenido")?<BookOpen size={18}/>:<Clock3 size={18}/>}</span><div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">{item.type} · {item.situation}</p><h3 className="mt-1 font-bold">{item.title}</h3><p className="mt-1 text-xs text-slate-500">{item.date}</p></div></article>)}
        </div>
      </section>
    </div>
  </>;
